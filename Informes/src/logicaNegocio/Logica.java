...
public class Logica {
    ...
    private void inicializarCorredoresApuntadosCarreras() {
        ...
        for (Carrera carrera : carreras) {
            if (carrera.getListaCorredores().size() > 0) {
    ...
    public ArrayList<Carrera> getCarrerasSinFinalizar() {
    ...
    public Carrera getCarrera(String id) {
        for (Carrera carrera : carreras) {
            if (carrera.getId().equalsIgnoreCase(id)) {
                return carrera;
            }
        }
        return null;
    ...
    public Corredor getCorredor(String dni) {
    ...
    public ArrayList<Carrera> getCarrerasCorredor(String dni) {
        ArrayList<Carrera> carreras = new ArrayList<>();
        for (TiemposCorredor c : corredoresApuntadosCarreras) {
            if (c.getDni().equalsIgnoreCase(dni)) {
                carreras.add(getCarrera(c.getIdCarrera()));
            }
        }
        if (carreras.size() > 0) {
            return carreras;
